...
import { gatherSourceContext, generateArtifactContent } from "../repository/artifact-generation.js";
import { createArtifactRecord, deleteArtifactRecord, findArtifactById, findArtifactByIdAndWorkspaceId, findArtifactsByWorkspaceId, updateArtifactRecord, type ArtifactRecord } from "../repository/artifact.repository.js";
...
const ARTIFACT_TITLES: Record<ArtifactRecord["type"], string> = {
    SUMMARY: "Summary",
...
export async function createArtifactForWorkspace(
    ...
    const artifact = await createArtifactRecord({
        ...
        title:
            input.title ||
            `${ARTIFACT_TITLES[input.type]} · ${new Date().toLocaleDateString()}`,
