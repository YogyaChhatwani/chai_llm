...
import { createConversationRecord, deleteConversationRecord, findConversationByIdAndWorkspaceId, findConversationsByWorkspaceId, touchConversation, updateConversationRecord } from "../repository/conversation.repository.js";
...
export async function streamWorkspaceChat(
    ...
    const citations = retrievedChunks.map((chunk: RetrievedChunk) => ({
    ...
    const systemPrompt = buildChatSystemPrompt({
        ...
        userMemories: userMemories.map((memory: { memory: string }) => memory.memory),
        webSearchEnabled,
    });
